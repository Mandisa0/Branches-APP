...
const buyItem = (item: string, value: number, cost: number) => {
  ...
  if (getStat('gold') - cost < 0) {
    toastText = "You don't have enough gold";
  } else {
...
const getRewardItem = (item: string, value: number, cost: number) => {


  showRewardAd((reward) => {
  console.log(`Player earned ${reward.amount} ${reward.type}`);
  buyItem(item, value, 0);
  });
